...
// Modifications by Thad Robertson
// -------------------------------
// + sensor detection
...
// value overwhich the sensor triggers a message to Processing
int threshold = 500;
// time interval between between 1st HIGH reading on pinOUT and triggering a sensor event in Processing if
// pinOUT is still HIGH
int delayTime = 1000;
// number of times in a row to send the sensor trigger up the line to the Processing sketch. 
// thinking is to account for timing mismatches i.e. Arduino sends and Processing fails to read because its in
// a different part of its cycle
int timesToSend = 20;
int repeats = 0;
long readTime = 0;
boolean triggerEvent, stored = false;
  ...
  for(;;) {
    ...
    switch(mode) {
     ...
     case MODE_HOLD:
      ...
      // check for sensor and use a delay already present in the original code
      // to prevent the serial buffer filling up. This delay *may* not prove
      // to be long enough so another solution may have to be sought.
      checkSensor();
...
void checkSensor(){
  // Check sensor input and send a note to Processing to change videos
 if (digitalRead(pinOUT) == HIGH) {
    //if nothing stored, store time and mark as stored
    if (!stored) {
    stored = true;
    readTime = millis();
  }

  //if time is stored, compare current time with stored time  
   if (stored) {
  ...
  if (!triggerEvent) stored = false;
 }

 if (triggerEvent) {
     if (repeats >= 1) {
        Serial.print('H'); // state = 72 on the Processing side
        repeats--;
     } else {
       stored = false;
       triggerEvent = false;
...
void enableSensor()
{  
  //To start the sensor reading, enable pin PinEN with HIGH signal
